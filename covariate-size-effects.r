...
select_covariate_effects <- function(){
  ...
  if (dim(out$alpha)[2] == 6){
    prepped_data = prepare_intervention_effects(out)
  } else {
    prepped_data = prepare_mobility_effects(out, formula_pooling, formula_partialpooling)
  }
...
prepare_intervention_effects <- function (out){
  ...
  return (c("data"=data, "plot_labels"=plot_labels))
}

prepare_mobility_effects <- function (out, formula_pooling, formula_partialpooling) {
  ...
  plot_labels_partial = regmatches(formula_partialpooling, gregexpr("[a-zA-Z]+", formula_partialpooling))[[1]]
  
  alpha = data.frame(as.matrix(out$alpha))
  # alpha_partial = data.frame(as.matrix(out$alpha_state))

  colnames(alpha) = plot_labels
  # colnames(alpha_partial) = plot_labels_partial
  data1 = bayesplot::mcmc_intervals_data(alpha,prob=.95,transformation=function(x) 1-exp(-x),point_est="mean")
  data1$type = "pooled"
  # data2 = bayesplot::mcmc_intervals_data(alpha_partial[,],prob=.95,transformation=function(x) 1-exp(-x),point_est="mean")
  # data2$type = "partial"
  # data = rbind(data1,data2)
  return (c("data"=data1, "plot_labels"=plot_labels))
